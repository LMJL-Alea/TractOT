...
for i,img in enumerate(img_dwi):
    if i==0:
        dwi,header= load_nifti(path+img+subject+'.nii.gz')
        bval,bvec=bval,bvec=read_bvals_bvecs(path+img+subject+'.bval',path+img+subject+'.bvec')
    else:
        dwi_i,header_i= load_nifti(path+img+subject+'.nii.gz')
...
save_nifti(path+'DTI_raw_PA_'+subject+'.nii.gz',dwi,header)
np.savetxt(path+'DTI_raw_PA_'+subject+".bval", bval.astype(int),fmt='%i', newline=" ")
np.savetxt(path+'DTI_raw_PA_'+subject+".bvec", bvec.T, newline="\n", delimiter="  ")
