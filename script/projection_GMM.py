...
def update_S(P,w1,S1,S2,eps):
    k2=P.shape[-1]
    return gaussians_barycenter_fixpoint(np.repeat(S1[:,:,None,:,:],k2,axis=2), P, num_iter=30, eps=eps,Sigma_init=S2)

def proj_GMM_MAS(k2,ksize,w1,S1,itr=10,eps=1e-7):
...
def gaussians_barycenter_fixpoint(S, P, num_iter=30, eps=1e-7,Sigma_init=None):
    ...
    for it in range(num_iter): #scale with d not with K
