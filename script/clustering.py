...
def update_w_S(Pweight,w1,S1):
    #Quantization of weights and covariance of anisotropic compartments
    ksize=S1.shape[2]
    nb_centroid=Pweight.shape[1]
    nb_MAP=S1.shape[1]
    w2,S2=np.zeros((nb_centroid,nb_MAP,ksize)),np.zeros((nb_centroid,nb_MAP,ksize,3,3))
    for i in range(nb_MAP):
        w1_temp=(w1[:,i,None]*Pweight[:,:,None]).transpose((1,0,2))
        kmax=np.max(np.count_nonzero(w1_temp,axis=-1),axis=-1)
        S1_temp=np.repeat(S1[None,:,i],nb_centroid,0).reshape(nb_centroid,-1,3,3)
        w2[:,i],S2[:,i]=proj_GMM_MAS(kmax,ksize,w1_temp.reshape(nb_centroid,-1),S1_temp,itr=10,eps=1e-5)
    return w2,S2

def clustering_MASs(nb_centroid,m1,wI1,I1,w1,S1,itr=20,eps=1e-7,alpha=0.5):
    ...
    for i in range(itr):
        ...
        w2_new,S2_new=update_w_S(Pweight,w1,S1)
...
def clustering_tract(nb_centroid,m1,itr=20,eps=1e-7,alpha=0.5):
